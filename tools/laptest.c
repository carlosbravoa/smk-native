...
static void shell_checks(smk_rom *rom)
{
    ...
    smk_ui_step(&ui, rom, &go);
    check(ui.screen == SMK_UI_PLAYER, "time trial -> driver");
    smk_ui_step(&ui, rom, &go);
    ...
    /* Grand Prix is refused, not entered */
    smk_ui_init(&ui);
    smk_ui_step(&ui, rom, &go);
    smk_ui_step(&ui, rom, &down);          /* onto GRAND PRIX */
    check(ui.mode_sel == 0, "the cursor reaches Grand Prix");
    smk_ui_step(&ui, rom, &go);
