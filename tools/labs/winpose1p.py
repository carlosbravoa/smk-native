...
L.sw(P1 + 0xC0, LAST << 8); L.sw(P1 + 0xF8, LAST << 8)
# P1 first: order table $010E and its rank word
L.sw(0x010E, 0x1000); L.sw(P1 + 0xE6, 0)
log("moving at %d; lap forced; rank word $%04X; frame now %s" % (L.speed(), L.w(P1 + 0xE6), which_frame()))
t0 = time.time(); f = 0; fin = None
while time.time() - t0 < 2400:
    L.flow(1) if fin is None else L.frame(0); f += 1
    if fin is None and (L.w(P1 + 0xC0) >> 8) > LAST:
    ...
    if fin is not None:
        ...
        if g in (10, 60, 120, 200, 280):
            for nm, buf in (("vram", L.b.vram), ("cgram", L.b.cgram), ("oam", L.b.oam)):
                open("tmp/win1p_%s_%d.bin" % (nm, g), "wb").write(bytes(buf))
        if g % 10 == 0 or g < 6:
