#!/usr/bin/env python3
"""Boot straight into cup/course like coursedump.py, settle, and save the
whole of WRAM too - the $1800 entity blocks for bug 14's position diff."""
import sys, os, time
...
c.run_frames_scanline(int(os.environ.get("SETTLE", "300")))
tag = "_c%d%d" % (cup, course)
open("tmp/wram%s.bin" % tag, "wb").write(bytes(b.wram))
log("track $%02X theme $%02X wram saved%s" % (b.wram[0x0124], b.wram[0x0126], tag))
