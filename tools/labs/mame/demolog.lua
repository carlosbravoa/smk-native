...
local FIELDS = {0x10,0x12,0x18,0x1C,0x1F,0x26,0x28,0x2A,0x60,0xA0,0xA2,0xA4,0xA6,0xA8,0xAA,0xAC,0xAE,0xB0,0xB2,0xB4,0xC2,0xC4,0xCA,0xD6,0xDE,0xE0,0xE2,0xE4,0xEA,0xFA,0xFC,0xE8,0xEE,0xB8}
local hdr = {"frame","kart","g28","g2A","g2C","g2E","g30","gE00","gE02","r08a","r08b"}
...
emu.register_frame_done(function()
  ...
  if started then
    ...
    for _, base in ipairs({0x1000, 0x1100}) do
      local row = {tostring(logged), string.format("%x", base), tostring(mem:read_u16(0x7E0028)), tostring(mem:read_u16(0x7E002A)), tostring(mem:read_u16(0x7E002C)), tostring(mem:read_u16(0x7E002E)), tostring(mem:read_u16(0x7E0030)), tostring(mem:read_u16(0x7E0E00)), tostring(mem:read_u16(0x7E0E02)), tostring(r08[0x80ABA7] or -1), tostring(r08[0x80ABB2] or -1)}
