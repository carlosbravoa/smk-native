...
| S12 | `src/main.c` entities | pipes draw in the ROM's own art (near tier) scaled continuously; movers are static | the sheet stores a size TIER per distance band and the game picks one; per-track handler table `$84:DD15` drives motion | P5 |
