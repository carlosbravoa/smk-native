...
| S5 | `src/mode7.c` sky | **part**: the sky is the backdrop colour (palette 0) and outside the plane repeats character 0, both measured from a real frame (NOTES 114).  Missing: the horizon HILL art, drawn above an HDMA `$2105` mode split | per-theme horizon graphics | P5 |
| S6 | `src/kart.c` bounce | **decoded**: the bounce is a ballistic launch (`$80F8C0` sets `$26`=$0080), not a timed knockback (NOTES 045); the wall hit's ~9-frame velocity freeze is MEASURED by displacement (NOTES 092) - `$42,x` turned out to be the HUD rank timer, not a window (NOTES 112) | per-class differences; the horizontal knockback magnitude | P3 residual |
...
| P5 race furniture | **part** — ground objects stamped with the ROM's own tiles (NOTES 074), sprite-obstacle entity list decoded and colliding (NOTES 078), HUD set + clock + lap counter on the game's own art, start countdown (NOTES 085).  hazard classes decoded and ported - water ($22) wade/skim, the fall ($24/$26) and Lakitu's rescue chain (NOTES 113).  Residual: the horizon/backdrop (S5), entity motion handlers, item behaviour, Lakitu's art, the splash/sink effects |
