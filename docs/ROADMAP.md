...
**S50 — The finish's five seconds, the self-driven kart after the line and
the stuck driver's hand-over are OURS (NOTES 297).**  Once everybody
playing is home the results come five seconds after the last of them -
three of celebration, two of fade - and the fifteen-second cooldown only
holds while a player is still out; a finished kart is driven by the
autopilot through the celebration, standing in for the original's CPU
take-over, whose timing is not measured; a trained driver that makes no
ground for three seconds hands over for five.  The user's asks, all
three.  The mole's shake-off (six alternations) stays OURS and
unmeasured; the mole's grab, ride and the shrunk kart are now the
recordings' (NOTES 297).

**S49 — Custom courses: the package, the generator's rules and the CUSTOM
column are OURS; everything a package feeds is the ROM's (docs/TRACKS.md).**
The generator's targets are measured on the twenty originals (waypoints
8 px into the next sector, ~97 px apart, paint 2 cells past the road)
and one of its rules is labelled as unpredictable: the bend at a
waypoint picks the ROM's own speed row on 43% of its 743 waypoints at
best, so the rows are assigned by the quantiles that reproduce the ROM's
mix and the author's line.txt is the real knob.  A package on a theme
stages that theme's first ROM slot before its tileset, so tiles the map
never uses differ from a slot's (the expander reading past its stream);
every used tile is byte-exact.  A course that crosses itself needs a
spine the tool does not take yet.

**S47 — The water is measured; the void's Lakitu is the older capture (NOTES 283).**
The wade steers and moves as the game's (`$80A892`'s state-8 row, drive
state 8's throttle), `$CA` is `$102`, the sink lifts the kart `$40` a
frame to `$700` after 34 still frames, and the sink's sprites and
Lakitu's rows are transcribed from the user's recordings' OAM.  OURS:
Lakitu's retreat when the kart escapes early.  The void (Ghost Valley)
...
shows the finishers only, once the first is home - the game's way - down
the left in one view and down the middle of a split (NOTES 288 retired
the constant column), and the minimaps draw the karts as faces.  Faces, digits, expression toggle and number
...
| P4 sprites | **done** — the projection is derived once from the ROM's own DSP-1 geometry (NOTES 083/084); a racer's height is lifted at its own sprite scale (NOTES 288): depth(L)=4972/(L-20.36), scale=depth/256 (ratio = Les, the cross-check), camera trails the kart 61 px.  Pose ladder measured pixel-exact (NOTES 080/081).  Residual: kart-sheet rows 1-2 purpose, sprite size quantisation (ours is continuous, labelled) |
| P5 race furniture | **part** - the live phase — ground objects stamped with the ROM's own tiles (NOTES 074), sprite-obstacle entity list decoded and colliding (NOTES 078), HUD set + clock + lap counter on the game's own art, start countdown (NOTES 085).  hazard classes decoded and ported - water ($22) wade/skim, the fall ($24/$26/$20/$28) and Lakitu's rescue as the ROM's own three states with a latched target (NOTES 113, 124); the wade, the sink and Lakitu's fishing measured frame by frame from the user's recordings (NOTES 283); the deep classes' first-frame drop and every fall's own sound forced in the oracle (NOTES 297).  The mole's grab, its ride and the shrunk kart are drawn as the recordings show them (NOTES 297).  Breakable blocks done and gated for both themes (NOTES 123/123a).  The sector map now matches the game's own $7F:5000 on all painted cells.  Lakitu's own art is now decoded and drawn for the start (NOTES 162).  Residual: the horizon/backdrop (S5), entity MOTION (S12), item behaviour, the splash/sink effects |
| P6 opponents | **driving and competitiveness in, personality not.**  The field reads the flow byte in sector 0 too, so it no longer cuts Ghost Valley 3's start corner across the void; its laps there are within 3% of the game's own, measured in the oracle (NOTES 294).  Flow-field steering (95% byte-exact), ramp launches, wall escapes and a Lakitu rescue get the field round **20/20** GP courses.  Kart-to-kart contact is the ROM's, weight classes and all (NOTES 166).  The rubber band is the ROM's own row chooser, `$80ADA0`, rebuilt in NOTES 174 and reproducing the game's choice on 94.2% of 39,074 recorded kart-frames - it turns on whether the neighbouring kart is the HUMAN, and its catch-up distances re-tune every lap.  The field takes the boost pads and flies the ramps as the game's karts do (NOTES 280: Mario Circuit 2's crossing, 31 of 31 in the recording), and skims shallow water as the game's field does on Donut Plains 3 (NOTES 297, measured in the oracle).  **The VS CPU opponent is a trained neural network** (S44): it races through the player physics by pressing buttons, decides every four frames, and beats the scripted driver on courses it never trained on; the scripted `src/autopilot.c` remains as its fallback.  The field's pace is the ROM's now: the AI sheds speed through `$80B064`'s rates instead of snapping to its target, and the four handicap karts take `$80B099`'s bonus in place of the rank penalty (NOTES 277/278, gated on the user's 50/100/150cc recordings).  Residual: the "in trouble" test (`$84`, `$10` bit 5) is approximated - `$84` is now known to be the shrink/hit timer; the distance CACHE is not modelled; per-kart driving personality |
| P7 audio | **effects DONE and decoded** (NOTES 211-246; a spin's $2A is its END, its whirr is sample $00 walked on a measured pitch triangle on a voice of its own, the squash's four sounds, NOTES 292/293; the engine's rev takes the game's knocks - halved on every wall, pipe, Thwomp and contact, zeroed through a fall and its rescue and while squashed, falling off-road, and its volume follows the note and halves under water, NOTES 285/287/289/290) — the game's own ids, rendered from its own BRR samples off the chip; four engines by driver pair, six rough surfaces, the overtake voices, the held voices (engine/roulette/skid).  **Music PARKED** by the user and off by default.  Residual: nine PENDING items listed under "Where to pick up next" (1a-1i) — the bridge's rate, `$4F`/`$53`, coverage 35/71 |
| P8 modes / menus | **three modes, two players** — the shell: title → players → mode → class → driver → course-by-cup → race → results.  GRAND PRIX runs the cup: the ROM's 9/6/3/1 to the top four, a points screen and an animated championship, the next grid from the last race's order and the coins by slot (NOTES 198/274/275), no retry - fifth or worse scores nothing and the cup goes on - the trophy.  The finishing list is on screen all race, the ROM's own faces and digits (NOTES 282); fifteen seconds after the fourth kart is home the race is over at the positions held while a player is still out (S46), and five seconds after the last player is home otherwise, the kart driving itself over the line (S50).  SINGLE RACE is one cup course: eight karts, the ROM's per-character grid order (`$81EE97`, NOTES 111) with the player at the back (NOTES 164).  TIME TRIAL is alone with one mushroom and keeps the top five lap times per course on disk.  1P / VS CPU (the neural driver, S44) / VS 2P side by side (S42).  Font, palettes, cup order, course names, lap count and the time-trial rules are all ROM-derived (NOTES 147/148).  Residual: the real menu art (S20), the mushroom grant rule (S19) |
| P9 custom courses | **built to T4** (docs/TRACKS.md) — packages, the CUSTOM column, the generator benchmarked on the twenty originals, `make trackcheck`, the role-map compiler with Rainbow Road's stripes and each theme's kerbs, one shareable `.smkt` file with the creator inside, and Track Studio: draw, place, pick a theme, build, validate, race the field, play.  Residual: a spine for courses that cross themselves, the airborne-reject flag, custom styles |
...
2. **The mole's second pose is not drawn** (NOTES 297).  The recording
   shows two 32x32 poses while a mole is up - `$C0`/`$C2` and
   `$C4`/`$C6` - in runs of 15-66 frames; the port draws the first
   only, because with four moles cycling at once the OAM does not say
   which pose belongs to which stage without projecting each entity.
   The grab and the ride are measured and in.

3. **A rail is a wall in the port and a drop in the game** (NOTES 297).
   In the user's own Ghost Valley run the fall fired the frame the
   kart's centre reached the class-`$82` rail; the port bounces off it
   and breaks it, and only drops the kart once the hole is there.  The
   port is the KINDER of the two.  Not changed, because the user's
   report was the opposite ("one wheel off and fell") and the port
   already reads the same point on the same table as the game.

4. **A shell dropped standing still may not queue `$39` when a COM hits
   it** (the user: only the spin, not the TSH).  NOTES 296 measured a
   THROWN shell; the static one needs a forced-projectile rig.  Until
   then the port plays `$39` for both.
...
### P9 — Custom courses: the track engine  (BUILT to T4, docs/TRACKS.md)
A new course is new DATA in the game's own shapes, not a new engine: a
package directory (the tilemap of indices, a theme, stamps, entities,
the finish strip and grid) plus the AI's own structures - the 64x64
sector map, the racing line, the direction field, the spawn segments -
GENERATED by a tool from the map alone, so the ROM's own AI, lap rule
and rescue drive it unmodified.  [`docs/TRACKS.md`](TRACKS.md) has the
design, the measurements and the status.  Built: the source struct and
the two loaders split into read + build (T0, round-tripped byte-exact on
the twenty GP courses by the selftest), the registry with a CUSTOM column
and records by name (T1), the generator with its benchmark against the
twenty originals (T2), `make trackcheck` (T3), the role-map compiler
(T4).  Not built: the oracle slot (T5), custom styles (T6), a spine for
courses that cross themselves (Mario Circuit 2 regenerates wrong and
says so).
