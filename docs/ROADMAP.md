...
### P9 — Custom courses: the track engine  (DESIGNED, docs/TRACKS.md)
...
and rescue drive it unmodified.  Phases T0-T6, each with a gate, in
[`docs/TRACKS.md`](TRACKS.md): T0 round-trips the 20 ROM courses through
the format byte-identically; T1 makes a package playable (registry,
CUSTOM column, records by id); T2 calibrates the generator against the
20 originals (its rules are OURS, their targets the ROM's data, measured
there); T3 turns the gate into `make trackcheck`; T4 compiles a role
map into tiles; T5 injects a package into a battle-arena slot of a
patched ROM so the real game plays it in the oracle.  Nothing built yet.
