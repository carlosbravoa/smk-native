...
| S15 | `src/main.c` `draw_entity` | **RESOLVED, and it was never an art problem.**  The near size is the game's own scale `$4200 / zf` read as 8.8 (NOTES 129): at the distance NOTES 139 measured, `12 x 16 * ($4200 / (256 * 34))` = **23.3 x 31.1** against the measured 23 x 31.  The port had been drawing every band at a FIXED size magnified 2x, inventing a missing art source to explain a number that falls straight out of the scale law - and that fixed size is what made a pipe 342 px away and one 3 px away both draw about 24-32 px (NOTES 154a) | same | closed |
